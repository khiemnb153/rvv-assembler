from src.supported_features import SUPPORTED_INSTS, MOPS
...
def translate_text(text):
    ...
    for inst in text:
        ...
        if group.startswith('v_arith'):
            ...
            icb = icb_of_v_arith(inst['format'], funct6, inst['vd'], inst['vs2'], vs1, rs1, imm, inst['vm'])
            
        elif group.startswith('v_load'):
            mop = MOPS[inst['op']]
            rs2 = inst['rs2'] if 'rs2' in inst else None
            vs2 = inst['vs2'] if 'vs2' in inst else None
            icb = icb_of_v_load(mop, inst['width'], inst['vd'], inst['rs1'], rs2, vs2, inst['vm'])

        elif group.startswith('v_store'):
            mop = MOPS[inst['op']]
            rs2 = inst['rs2'] if 'rs2' in inst else None
            vs2 = inst['vs2'] if 'vs2' in inst else None
            icb = icb_of_v_load(mop, inst['width'], inst['vs3'], inst['rs1'], rs2, vs2, inst['vm'])
